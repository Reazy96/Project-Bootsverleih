...
const ReservierungsUebersicht = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:3010/api/v1/reservierung")
      ...
      .catch((error) => console.error("Error fetching all Reservierungen", error));
  }, []);

  return (
    <section>
      <AddReservierung reservierungen={reservierungen} setReservierungen={setReservierungen} />
      ...
      {reservierungen?.map((item, index) => (
        <div key={index}>
          ...
          <p>{item?.Startdatum}</p>
          <p>{item?.Enddatum}</p>
          <EditReservierung reservierungItem={item} reservierungen={reservierungen} setReservierungen={setReservierungen} />
          <button onClick={() => DeleteClick}>❌ Löschen</button>
        </div>
      ))}
    </section>
  );
